<script lang="ts">
  ...
  import { CONFIG_PATH, loadDashboardConfig } from "./lib/utils/config/dashboardConfig";
...
>
  <div class="m-auto flex items-center justify-between align-middle md:max-w-(--breakpoint-2xl)">
    <div>
    ...
    <nav>
      <ul class="flex items-center">
        <li>
          <input
            class="mr-1 hidden rounded-md border bg-white p-1 shadow-xs sm:block dark:border-mine-shaft-900 dark:bg-mine-shaft-950 dark:text-zinc-100"
            type="search"
            placeholder="Search..."
        ...
        <li>
          <ThemeToggle
            ...
            handleSwitchTheme={() => {
              setTheme(colorTheme);
              colorTheme =
                colorTheme === COLOR_THEMES.DARK ? COLOR_THEMES.LIGHT : COLOR_THEMES.DARK;
            }}
          />
        </li>
      </ul>
    ...
    {#await dashboardConfig then dashboardResult}
      {#if !dashboardResult.success}
        <div class="rounded-sm bg-white p-2 shadow-sm dark:bg-mine-shaft-950">
          <h1 class="text-lg font-semibold text-red-500">Error loading configuration</h1>
          <pre class="text-wrap dark:text-zinc-100">{dashboardResult.error.message}</pre>
        </div>
      {:else}
        <input
          class="mb-4 rounded-md border bg-white p-1 shadow-xs sm:hidden dark:border-mine-shaft-900 dark:bg-mine-shaft-950 dark:text-zinc-100"
          type="search"
          placeholder="Search..."
          bind:value={searchTerm}
        />
